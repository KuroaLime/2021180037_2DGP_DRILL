...
character_kick = load_image('character_kick.png')
...
def char_walk(x,frame):
    while (x < 800):
        ...
        character_walk.clip_draw(frame * 80, 0, 80, 110, x, 80)
        update_canvas()
        frame = (frame + 1) % 6
        x += 5
        delay(0.01)
...
def char_kick(x,frame):
    while (x < 800):
        clear_canvas()
        grass.draw(400, 30)

        character_kick.clip_draw(frame * 113, 0, 113, 150, x, 105)
        update_canvas()
        frame = (frame + 1) % 14
        x += 10
